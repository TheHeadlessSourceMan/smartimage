...
def applyMask(image,mask):
	...
	if hasattr(image,'immutable') and image.immutable==True:
		image=image.copy()
	# TODO: will this work with images that have existing alpha??
	image.putalpha(mask)
	return image
...
def paste(image,overImage,position=(0,0),resize=True):
	...
	if (image.width+position[0]>overImage.width) or (image.height+position[1]>overImage.height):
		newImg=Image(
			(max(image.width+position[0]>overImage.width).max(image.height+position[1]>overImage.height)),
